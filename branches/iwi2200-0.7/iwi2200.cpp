...
bool darwin_iwi2200::start(IOService *provider)
{
	...
	do {
		...
		fTransmitQueue->setCapacity(1500);//IPW_RX_BUF_SIZE);
				   ...
				   struct ieee80211_rx_stats *stats)
{
	...
	if (priv->config & CFG_NET_STATS) {
		...
		//fNetif->
		inputPacket(skb);//,mbuf_len(skb),IONetworkInterface::kInputOptionQueuePacket);
		 ...
		 struct ieee80211_rx_stats *rx_stats)
{
	...
	if (skb) {
		...
		//inputPacket(skb);
		//int ri=fNetif->inputPacket(skb,mbuf_len(skb),IONetworkInterface::kInputOptionQueuePacket);
		inputPacket(skb);
		//if (ri!=0) fNetif->flushInputQueue();
