...
bool darwin_iwi2200::start(IOService *provider)
{
	...
	do {
		...
		fTransmitQueue->setCapacity(1500);
...
void darwin_iwi2200::ipw_rx_queue_reset(struct ipw_priv *priv, struct ipw_rx_queue *rxq)
{
	...
	for (i = 0; i < RX_FREE_BUFFERS + RX_QUEUE_SIZE; i++) {
		...
		if (rxq->pool[i].skb != NULL) {
			...
			if (mbuf_type(rxq->pool[i].skb)!= MBUF_TYPE_FREE) mbuf_freem_list(rxq->pool[i].skb);
			rxq->pool[i].skb = NULL;
		}
		list_add_tail(&rxq->pool[i].list, &rxq->rx_used);
	}
...
void darwin_iwi2200::ieee80211_txb_free(struct ieee80211_txb *txb)
{
	...
	for (i = 0; i < txb->nr_frags; i++)
		...
		{
			if(mbuf_type(txb->fragments[i])!=MBUF_TYPE_FREE) mbuf_freem_list(txb->fragments[i]);
		 ...
		 * Also start the clocks. */
		 if (pl>0)//==2)
		 {
			...
			if (rc) {
				...
				queue_te(1,OSMemberFunctionCast(thread_call_func_t,this,&darwin_iwi2200::ipw_adapter_restart),priv,2,true);
				return rc;
			}
		 }
...
IOReturn darwin_iwi2200::getHardwareAddress( IOEthernetAddress * addr )
{
	...
	if (priv)
	{
		...
		IWI_DEBUG_FULL("getHardwareAddress " MAC_FMT "\n",MAC_ARG(priv->mac_addr));
	}
	return kIOReturnSuccess;
}
...
u8 darwin_iwi2200::ipw_add_station(struct ipw_priv *priv, u8 * bssid)
{
	...
	IWI_LOG("Adding AdHoc station: " MAC_FMT "\n", MAC_ARG(bssid));
				   ...
				   struct ieee80211_rx_stats *stats)
{
	...
	//struct ieee80211_hdr_4addr *header = (struct ieee80211_hdr_4addr *)(mbuf_data(skb));// + IPW_RX_FRAME_SIZE);
	...
	if (priv->config & CFG_NET_STATS) {
		...
		fNetif->inputPacket(skb,0,IONetworkInterface::kInputOptionQueuePacket);
...
void darwin_iwi2200::ipw_handle_data_packet(struct ipw_priv *priv,
				   ...
				   struct ieee80211_rx_stats *stats)
{
	if (mbuf_len(rxb->skb)==0 || rxb->skb==NULL) return;
	
	if (cacherec==NULL) 
	{
		mbuf_dup(rxb->skb,1,&cacherec);
		if (mbuf_len(cacherec)!=mbuf_pkthdr_len(cacherec)) 
		{
			if (rxb->skb!=NULL) if(mbuf_type(rxb->skb)!=MBUF_TYPE_FREE) mbuf_freem_list(rxb->skb);
			rxb->skb=NULL;
			return;
		}
		else
		{
			if (rxb->skb!=NULL) if(mbuf_type(rxb->skb)!=MBUF_TYPE_FREE) mbuf_freem_list(rxb->skb);
			rxb->skb=NULL;
			mbuf_dup(cacherec,1,&rxb->skb);
			if (cacherec!=NULL) if(mbuf_type(cacherec)!=MBUF_TYPE_FREE) mbuf_freem_list(cacherec);
			cacherec=NULL;
		}
	}
	else
	{
		if (mbuf_len(rxb->skb)<mbuf_pkthdr_len(cacherec))	
		{
			mbuf_copyback(cacherec, mbuf_len(cacherec), mbuf_len(rxb->skb),
					  mbuf_data(rxb->skb), 1);
			if (mbuf_len(cacherec)!=mbuf_pkthdr_len(cacherec)) 
			{
				if (rxb->skb!=NULL) if(mbuf_type(rxb->skb)!=MBUF_TYPE_FREE) mbuf_freem_list(rxb->skb);
	...
	if( mbuf_flags(rxb->skb) &  MBUF_PKTHDR) {
		...
		IWI_DEBUG_FULL("Pkhdr Rx packet of %d[%d] bytes.\n", mbuf_pkthdr_len(rxb->skb) ,mbuf_len(rxb->skb) );
	} else	{
		mbuf_setlen(rxb->skb, le16_to_cpu(pkt->u.frame.length));
		IWI_DEBUG_FULL("Nkhdr Rx packet of %d bytes.\n", mbuf_len(rxb->skb) );
	}
#endif	
...
void darwin_iwi2200::ipw_rx(struct ipw_priv *priv)
{
	...
	while (i != r) {
		...
		case RX_FRAME_TYPE:	/* 802.11 frame */  {
				...
				//header =(struct ieee80211_hdr_4addr *)(mbuf_data(rxb->skb) );//+  IPW_RX_FRAME_SIZE);
		...
		if (rxb->skb != NULL) {
			...
			if(mbuf_type(rxb->skb)!=MBUF_TYPE_FREE) mbuf_freem_list(rxb->skb);
			rxb->skb = NULL;
		}
							 ...
							 struct apple80211_status_dev_data *dd)
{
	...
	//ifnet_set_mtu(fifnet,3000); //>=IPW_RX_BUF_SIZE
	ipw_sw_reset(1);
			    ...
			    struct ieee80211_network *network, int roaming)
{
	...
	if (network->bssid)
	{
		for (int i=0;i<20;i++) 
		{
			...
			{
				IWI_DEBUG("Network '%s (%02x:%02x:%02x:%02x:%02x:%02x)' in exclude list. "
...
void darwin_iwi2200::ipw_gather_stats(struct ipw_priv *priv)
{
	...
	IWI_DEBUG_FULL("Missed beacon: %3d%% (%d%%)\n",
...
void darwin_iwi2200::ipw_handle_missed_beacon(struct ipw_priv *priv, int missed_count)
{
	...
	IWI_DEBUG_FULL("Missed beacon: %d\n", missed_count);
}
...
//#ifdef CONFIG_IPW2200_QOS -> config qos is default
	...
	*/		   
		...
		for ( mn = txb->fragments[i]; mn; mn = mbuf_next(mn) )
		{
			if(  mn = txb->fragments[i] ){
				...
				IWI_DEBUG_FULL(" pkt_len %d mbuf_len %d \n",
...
UInt32 darwin_iwi2200::outputPacket(mbuf_t m, void * param)
{
	...
	if (offset) { 
		nm = copyPacket(m, 0);
		if (m!=NULL && mbuf_type(m)!=MBUF_TYPE_FREE) freePacket(m);
	}
		else    nm=m;
	...
	{
		if (nm!=NULL && mbuf_type(nm)!=MBUF_TYPE_FREE) freePacket(nm);
			return kIOReturnOutputDropped;
	}
							      ...
							      u8 * dst)
{
	...
	for (i = 0; i < IEEE80211_FRAG_CACHE_LEN; i++) {
		...
		if (entry->skb != NULL /*&&
			...
			if(mbuf_type(entry->skb)!=MBUF_TYPE_FREE) mbuf_freem_list(entry->skb);
			entry->skb = NULL;
		}
						...
						struct ieee80211_hdr_4addr *hdr)
{
	...
	if (frag == 0) {
		...
		if (entry->skb != NULL)
			if(mbuf_type(entry->skb)!=MBUF_TYPE_FREE)
