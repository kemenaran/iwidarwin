...
inline void mbf(mbuf_t m)
{
	if (m!=NULL)
	if (mbuf_len(m)!=0 && mbuf_type(m)!=MBUF_TYPE_FREE)	mbuf_freem_list(m);
	//m=NULL;
}
...
//FIXME: jiffies are used by some functions. how should this be set? 500 jiffies = 1 second??
#define jiffies		\
({		\
	uint64_t m;		\
	clock_get_uptime(&m);		\
...
virtual void	dataLinkLayerAttachComplete( IO80211Interface * interface );
	...
	virtual void ipw_send_disassociate(int quiet);
