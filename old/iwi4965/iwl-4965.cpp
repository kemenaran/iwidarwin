...
static void iwl4965_bg_statistics_periodic(unsigned long data)
{
	...
	clone->queue_te(15,OSMemberFunctionCast(thread_call_func_t,clone,&darwin_iwi4965::iwl4965_bg_statistics_work),priv,NULL,true);
 ...
 */
void darwin_iwi4965::iwl4965_bg_statistics_work(struct iwl_priv *priv)
{
...
static void iwl4965_bg_sensitivity_work(struct iwl_priv *priv)
{
	//struct iwl_priv *priv = container_of(work, struct iwl_priv,
	//						sensitivity_work);
...
void darwin_iwi4965::iwl4965_bg_txpower_work(struct iwl_priv *priv)
{
	//struct iwl_priv *priv = container_of(work, struct iwl_priv,
	//				     txpower_work);
...
int iwl_hw_tx_queue_free_tfd(struct iwl_priv *priv, struct iwl_tx_queue *txq)
{
	...
	for (i = 0; i < counter; i++) {
		...
		if (txq->txb[txq->q.last_used].skb[i]) {
			mbuf_t skb = txq->txb[txq->q.last_used].skb[i];
...
void iwl_hw_rx_statistics(struct iwl_priv *priv, struct iwl_rx_mem_buffer *rxb)
{
	...
	if (unlikely(!(priv->status & STATUS_SCANNING) &&
		...
		clone->queue_te(14,OSMemberFunctionCast(thread_call_func_t,clone,&darwin_iwi4965::iwl4965_bg_txpower_work),priv,NULL,true);
				       ...
				       struct ieee80211_rx_status *stats)
{
	...
	/* start from MAC */
	skb_reserve(rxb->skb, (u8 *)hdr - (u8 *)pkt);
				...
				struct iwl_rx_mem_buffer *rxb)
{
	...
	u16 fc;
	struct ieee80211_rx_status stats;// = {
		stats.mactime = rx_start->beacon_time_stamp;
		stats.freq = ieee80211chan2mhz(le16_to_cpu(rx_start->channel));
		stats.channel = rx_start->channel;
		stats.phymode =
			(rx_start->phy_flags & RX_RES_PHY_FLAGS_BAND_24_MSK) ?
			MODE_IEEE80211G : MODE_IEEE80211A;
		stats.antenna = 0;
		stats.rate = rx_start->rate.s.rate;
		stats.flag = rx_start->phy_flags;
	//};
	u8 network_packet;
...
static void iwl4965_rx_reply_compressed_ba(struct iwl_priv *priv,
					   struct iwl_rx_mem_buffer *rxb)
{
	struct iwl_rx_packet *pkt = (void *)mbuf_datarxb->skb);
...
{
	clone->queue_te(14,OSMemberFunctionCast(thread_call_func_t,clone,&darwin_iwi4965::iwl4965_bg_txpower_work),NULL,NULL,false);
	clone->queue_te(15,OSMemberFunctionCast(thread_call_func_t,clone,&darwin_iwi4965::iwl4965_bg_statistics_work),NULL,NULL,false);
