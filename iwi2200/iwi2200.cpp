...
bool darwin_iwi2200::start(IOService *provider)
{
	...
	do {
		...
		//setProperty(kIOMaxPacketSize, 100000);
...
int darwin_iwi2200::ipw_tx_skb(struct ipw_priv *priv, struct ieee80211_txb *txb, int pri)
{
	...
	*/		   
		...
		if(mbuf_next(txb->fragments[i]))
		{
			IWI_ERR("chains in txb->fragments\n");
			mn=mergePacket(txb->fragments[i]);
			txb->fragments[i]=mn;
		}
		if (txb->fragments[i])
		{
...
mbuf_t darwin_iwi2200::mergePacket(mbuf_t m)
{
	...
	if  (mbuf_pkthdr_len(nm) <= 0 ) {
		...
		IWI_WARN("output packet is not single chain but cannot merge to single\n");
	} else {
		mbuf_setlen(nm,0);
