...
class darwin_iwifi : public IO80211Controller
#else
class darwin_iwifi : public IOEthernetController
#endif
    {
        OSDeclareDefaultStructors(darwin_iwifi)
        
    public:
        virtual void queue_td2(int num , thread_call_func_t func);
		virtual void queue_te2(int num, thread_call_func_t func, thread_call_param_t par, UInt32 timei, int start);
        ...
        virtual int		addMediumType(UInt32 type, UInt32 speed, UInt32 code, char* name = 0);
		...
		virtual int attachInterfaceWithMacAddress( void * macAddr, 
                                                   UInt32 macLen, 
    ...
    virtual void setPowerStateOn(void);
		virtual IOReturn setMulticastMode(int active);
