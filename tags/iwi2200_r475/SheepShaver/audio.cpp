...
{
	D(bug(" AudioSetInfo %d %c%c%c%c, infoPtr %08lx, source ID %08lx\n", selector,selector >> 24, (selector >> 16) & 0xff, (selector >> 8) & 0xff, selector & 0xff, infoPtr, sourceID));
	M68kRegisters r;
	switch (selector) {
		case siSampleSize:
		...
		case siSpeakerVolume:
			//(printf("  set speaker volume %08lx\n", infoPtr));
			audio_set_speaker_volume(infoPtr);
			break;

		case siHardwareMute:
			//(printf("  set hardware mute\n"));
			audio_set_main_mute((uint16)infoPtr);
			break;

		case siHardwareVolume:
			//(printf("  set hardware volume %08lx\n", infoPtr));
...
int16 SoundInControl(uint32 pb, uint32 dce)
{
	...
	//(printf(" SoundInControl %c%c%c%c\n", selector , selector >> 8, selector >> 16, selector >>24));
	switch (selector) {
		default:
			return noErr;//-231;	// siUnknownInfoType
	}
}
...
int16 SoundInStatus(uint32 pb, uint32 dce)
{
	...
	uint32 selector = param[0];
	//D(bug
	//(printf(" SoundInStatus %c%c%c%c\n", selector , selector >> 8, selector >> 16, selector >>24));
	char ssel[4];
	sprintf(ssel,"%c%c%c%c", selector , selector >> 8, selector >> 16, selector >>24);
	switch (FOURCC(ssel[0],ssel[1],ssel[2],ssel[3])) {
#if 1
