...
int darwin_iwi2200::ipw_sw_reset(int option)
{
	...
	if (!net_dev) {
		IWI_DEBUG("Unable to network device.\n");
		return -1;
	}
	...
	if (associate)
		...
		IWI_DEBUG("Auto associate disabled.\n");

	if (auto_create)
		priv->config |= CFG_ADHOC_CREATE;
	else
		IWI_DEBUG("Auto adhoc creation disabled.\n");
	...
	if (disable2 && option) {
		...
		IWI_DEBUG("Radio disabled.\n");
	}

	if (channel != 0) {
		priv->config |= CFG_STATIC_CHANNEL;
		priv->channel = channel;
		IWI_DEBUG("Bind to static channel %d\n", channel);
	...
	if ((fPCIDevice->configRead16(kIOPCIConfigDeviceID) == 0x4223) ||
		...
		if (option == 1)
			IWI_DEBUG(": Detected Intel PRO/Wireless 2915ABG Network "
			       "Connection\n");
...
bool darwin_iwi2200::start(IOService *provider)
{
	...
	do {
		...
		{
			IWI_DEBUG("ERR: getOutputQueue()\n");
			break;
		}
		fTransmitQueue->setCapacity(IPW_RX_BUF_SIZE);
		...
		//queue_te(7,OSMemberFunctionCast(thread_call_func_t,this,&darwin_iwi2200::ipw_up),NULL,NULL,false);
		
		
		pl=1;
		return true;			// end start successfully
	} while (false);
	IWI_DEBUG("%s ::start faile\n", getName());	
	stop(provider);
	free();
				  ...
				  struct clx2_tx_queue *txq)
{
	...
	for (i = 0; i < le32_to_cpu(bd->u.data.num_chunks); i++) {
		//pci_unmap_single(dev, le32_to_cpu(bd->u.data.chunk_ptr[i]), le16_to_cpu(bd->u.data.chunk_len[i]), PCI_DMA_TODEVICE);
		bd->u.data.chunk_ptr[i]=NULL;
		if (txq->txb[txq->q.last_used]) {
			//ieee80211_txb_free(txq->txb[txq->q.last_used]);
			int ii;
			if (unlikely(!txq->txb[txq->q.last_used]))
			{}
			else
			{
			for (ii = 0; ii < txq->txb[txq->q.last_used]->nr_frags; ii++)
				if (txq->txb[txq->q.last_used]->fragments[ii])
					//dev_kfree_skb_any(txq->txb[txq->q.last_used]->fragments[ii]);
					mbuf_freem_list(txq->txb[txq->q.last_used]->fragments[ii]);
			kfree(txq->txb[txq->q.last_used]);
			}
...
void darwin_iwi2200::ipw_rx_queue_replenish(void *data)
{
	...
	while (!list_empty(&rxq->rx_used)) {
		...
		rxb->dma_addr = mbuf_data_to_physical(mbuf_datastart(rxb->skb));	
...
int darwin_iwi2200::ipw_load(struct ipw_priv *priv)
{
 ...
 retry:
	...
	if (rc < 0) {
		IWI_DEBUG("Unable to load boot firmware\n");
		goto error;
	}
	...
	if (rc < 0) {
		IWI_DEBUG("device failed to boot initial fw image\n");
		//goto error;
	}
	...
	if(rc < 0) {
		IWI_DEBUG("%s ucode failed to upload\n", getName());
		return kIOReturnError;
	}
	...
	if (rc < 0) {
		IWI_DEBUG("Unable to load firmware\n");
		goto error;
	}
	...
	if (rc < 0) {
		IWI_DEBUG("Unable to initialize queues\n");
		goto error;
	}
...
//fNetif->setPoweredOnByUser(true);
	...
	pl++;
		...
		if (ipw_set_geo(priv->ieee, &ipw_geos[j])) {
			IWI_DEBUG("Could not set geography.");
			return 0;
		}
		IWI_DEBUG("geography %d = %s\n",j,ipw_geos[j].name);
		...
		ipw_down(priv); // fix me 
		
	}

	/* tried to restart and config the device for as long as our
	 * patience could withstand */
	IWI_DEBUG("Unable to initialize device after %d attempts.\n", pl);
	return -1;
}
...
UInt32 darwin_iwi2200::handleInterrupt(void)
{
	...
	// my version from ipw_isr
	/*
	// reauire normal spinlock
	r = ipw_read32(IPW_INTA_RW);
	inta_mask = ipw_read32(IPW_INTA_MASK_R);
	
	if (r == 0xffffffff) {
		IWI_DEBUG("%s IRQ INTA == 0xFFFFFFFF\n",__FUNCTION__);
		return false;
	}
	if(!(r & (IPW_INTA_MASK_ALL & inta_mask))) {
		IWI_DEBUG("%s not raise shared interrupt \n",__FUNCTION__);
		return false;
	}
	// disale interrupts
	if(priv->status & STATUS_INT_ENABLED)
	...
	r &= (IPW_INTA_MASK_ALL & inta_mask);
	ipw_write32(IPW_INTA_RW, r);
	
	
	// unlock normal
	// lock irqsave
	inta_mask = ipw_read32(IPW_INTA_MASK_R);
	r |=  (ipw_read32(IPW_INTA_RW) & ( IPW_INTA_MASK_ALL & inta_mask) );
	// unlock irqsave
	// lock irqsave
	...
	*/
	...
	{
		IWI_DEBUG("PHY_OFF_DONE Restarting\n");
		priv->status |= STATUS_RF_KILL_HW;
	...
	if (r & (IPW_INTA_BIT_FATAL_ERROR | IWI_INTR_PARITY_ERROR)) {
		IWI_DEBUG("Firmware error detected.  Restarting.\n");
		priv->status &= ~STATUS_INIT;
		priv->status &= ~STATUS_HCMD_ACTIVE;
		priv->status &= ~(STATUS_ASSOCIATED | STATUS_ASSOCIATING);
		queue_te(1,OSMemberFunctionCast(thread_call_func_t,this,&darwin_iwi2200::ipw_adapter_restart),priv,NULL,true);
	...
	if (r & IPW_INTA_BIT_RX_TRANSFER) {
		IWI_DEBUG("IPW_INTA_BIT_RX_TRANSFER\n");
		ipw_rx(priv);
		ret = IPW_INTA_BIT_RX_TRANSFER;
	}

	if (r & IPW_INTA_BIT_TX_CMD_QUEUE) {
		IWI_DEBUG("Commad Completed\n");
...
int darwin_iwi2200::__ipw_send_cmd(struct ipw_priv *priv, struct host_cmd *cmd)
{
	...
	fWorkLoop->runEventSources();
	if (fWorkLoop->onThread()) {
		IWI_DEBUG("current is fworkloop thread\n");
	}else{
		IWI_DEBUG("current is not fworkloop thread\n");
	}
	if(fInterruptSrc->isEnabled()){
		IWI_DEBUG("current is interrupt enable\n");
	}*/
...
int darwin_iwi2200::ipw_set_tx_power(struct ipw_priv *priv)
{
    ...
    /* if (fInterruptSrc->checkForWork()){
		IWI_DEBUG("checkForWork true \n");
	}else{
		IWI_DEBUG("checkForWork false \n");
	} */
...
{
	mbuf_t skb = rxb->skb;
	struct ipw_rx_packet *pkt = (struct ipw_rx_packet *)mbuf_datastart(skb);
	...
	struct ieee80211_hdr_4addr *header = (struct ieee80211_hdr_4addr *)(((UInt8*)(mbuf_datastart(skb))+ IPW_RX_FRAME_SIZE));
				      ...
				      mbuf_t skb)
{
	...
	hdr = (struct ieee80211_hdr *)mbuf_datastart(skb);
	...
	switch (priv->ieee->sec.level) {
	case SEC_LEVEL_3:
		IWI_DEBUG("ipw_rebuild_decrypted_skb level3");
		/* Remove CCMP HDR */
		memmove((UInt8*)mbuf_datastart(skb) + IEEE80211_3ADDR_LEN,
			(UInt8*)mbuf_datastart(skb) + IEEE80211_3ADDR_LEN + 8,
			(size_t)((UInt8*)mbuf_datastart(skb) - IEEE80211_3ADDR_LEN - 8));
		//skb_trim(skb, skb->len - 16);	/* CCMP_HDR_LEN + CCMP_MIC_LEN */
		mbuf_adj(skb,mbuf_len(skb)-16);
		break;
	case SEC_LEVEL_2:
		IWI_DEBUG("ipw_rebuild_decrypted_skb level2");
		break;
	case SEC_LEVEL_1:
		/* Remove IV */
		IWI_DEBUG("ipw_rebuild_decrypted_skb level1");
		memmove((UInt8*)mbuf_datastart(skb) + IEEE80211_3ADDR_LEN,
			(UInt8*)mbuf_datastart(skb) + IEEE80211_3ADDR_LEN + 4,
			(size_t)((UInt8*)mbuf_datastart(skb) - IEEE80211_3ADDR_LEN - 4));
		//skb_trim(skb, skb->len - 8);	/* IV + ICV */
		mbuf_adj(skb,mbuf_len(skb)-8);
		break;
	case SEC_LEVEL_0:
		IWI_DEBUG("ipw_rebuild_decrypted_skb level0");
		break;
	default:
				   ...
				   struct ieee80211_rx_stats *stats)
{
	...
	struct ipw_rx_packet *pkt = (struct ipw_rx_packet *)mbuf_datastart(rxb->skb);
	...
	mbuf_align_32(rxb->skb, le16_to_cpu(pkt->u.frame.length));
	
	IWI_DEBUG("Rx packet of %d bytes.\n", mbuf_len(rxb->skb));

	/* HW decrypt will not clear the WEP bit, MIC, PN, etc. */
	hdr = (struct ieee80211_hdr_4addr *)mbuf_datastart(rxb->skb);
	if (priv->ieee->iw_mode != IW_MODE_MONITOR &&
	    (ipw_is_multicast_ether_addr(hdr->addr1) ?
	     !priv->ieee->host_mc_decrypt : !priv->ieee->host_decrypt))
		ipw_rebuild_decrypted_skb(priv, rxb->skb);

	fNetif->inputPacket(rxb->skb,mbuf_len(rxb->skb),IONetworkInterface::kInputOptionQueuePacket);
	fNetif->flushInputQueue();
...
void darwin_iwi2200::ipw_rx(struct ipw_priv *priv)
{
	...
	while (i != r) {
		...
		pkt = (struct ipw_rx_packet *)mbuf_datastart(rxb->skb);
		IWI_DEBUG("Packet: type=%02X seq=%02X bits=%02X\n",
		...
		case RX_FRAME_TYPE:	/* 802.11 frame */  {
				...
				header = (struct ieee80211_hdr_4addr *)(((UInt8*)(mbuf_datastart(rxb->skb))+ IPW_RX_FRAME_SIZE));
				...
				case IEEE80211_FTYPE_MGMT:
					IWI_DEBUG("IEEE80211_FTYPE_MGMT\n");
	...
	hw = CSR_READ_4(memBase, IWI_CSR_RX_RIDX);
	IWI_DEBUG("RxQueueIntr: hw %x (%d) -> %d\n", hw, hw, rxq.cur);

	for(; rxq.cur != hw; ) {
		data = &rxq.data[rxq.cur];

		//data->memD->readBytes(0, &hdr, sizeof(struct iwi_hdr));
		pkt = (struct ipw_rx_packet *)data->m_data;
		IWI_DEBUG("Packet: type=%02X seq=%02X bits=%02X\n",
			     pkt->header.message_type,
			     pkt->header.rx_seq_num, pkt->header.control_bits);
		...
		//switch (hdr->type) {
			...
			default:
				IWI_DEBUG("default %d\n", pkt->header.message_type);//hdr->type);
				
		}
		
		rxq.cur = (rxq.cur + 1) % IWI_RX_RING_COUNT;
	}
	
	hw = (hw == 0) ? IWI_RX_RING_COUNT - 1 : hw - 1;
	CSR_WRITE_4(memBase, IWI_CSR_RX_WIDX, hw);*/
}
...
{
	IWI_DEBUG("trying to down the interface\n");
	if (pl==0)
	{
		super::disable(netif);
		//stop(NULL);
...
//ifnet_get_multicast_list(fifnet,&addresses);
//int rs=ifnet_get_address_list(fifnet,&addresses);
//int rs=ifnet_get_address_list_family(fifnet, &addresses, AF_INET);

	/* Protect our use of the network_list */
	//list_for_each_entry(network, &priv->ieee->network_list, list) ipw_best_network(priv, &match, network, 0);
	int i;
	//IWI_DEBUG("first address[%x] second address [%x]\n",addresses[0],addresses[1]);
	//IWI_DEBUG("rx0 queue address phys [%x] virt [%x]\n",(&rxq.data[0])->physaddr,(&rxq.data[0])->m_data);
	//IWI_DEBUG("rx1 queue address phys [%x] virt [%x]\n",(&rxq.data[1])->physaddr,(&rxq.data[1])->m_data);
	//IWI_DEBUG("ioBase(phy) [%x] memBase(vir?) [%x]\n",ioBase,memBase);

	/*INIT_LIST_HEAD(&priv->ieee->network_list);
	for (i = 0; i < MAX_NETWORK_COUNT; i++)
		list_add_tail(&priv->ieee->networks[i].list,
			      &priv->ieee->network_free_list);
				  

	for (i = 0; addresses[i] != NULL; i++) 
	{
		//inet_arp_init_ifaddr(fifnet,addresses[i]);
					
		list_add_tail(&(((ieee80211_network*)addresses[i])->list),&priv->ieee->network_list);
		//ipw_best_network(priv, &match, (ieee80211_network*)addresses[i], 0);
	}*/
	list_for_each_entry(network, &priv->ieee->network_list, list) ipw_best_network(priv, &match, network, 0);
	...
	//ifnet_free_multicast_list(addresses);
	...
	/*ifaddr_t tt=(ifaddr_t)network;
	//IWI_DEBUG("llad %d\n", ifnet_lladdr_copy_bytes(fifnet, tt, ETHER_ADDR_LEN));

	int r= ifnet_set_lladdr(fifnet, tt, ETHER_ADDR_LEN);
	if (r==0)
	{
		priv->status |= STATUS_ASSOCIATED;
		ipw_led_link_off(priv);
...
void darwin_iwi2200::ipw_adhoc_check(void *data)
{
	...
	queue_te(8,OSMemberFunctionCast(thread_call_func_t,this,&darwin_iwi2200::ipw_adhoc_check),priv,priv->assoc_request.beacon_interval,true);
}
				...
				struct ipw_rx_notification *notif)
{
	...
	case HOST_NOTIFICATION_STATUS_ASSOCIATED:{
			...
			case CMAS_ASSOCIATED:{
					IWI_DEBUG(						  "associated: '%s' %02x:%02x:%02x:%02x:%02x:%02x \n",
						  escape_essid((const char*)priv->essid,
							       priv->essid_len),
						  MAC_ARG(priv->bssid));
			...
			case CMAS_AUTHENTICATED:{
					...
					IWI_DEBUG(						  "authenticated: '%s' %02x:%02x:%02x:%02x:%02x:%02x\n",
						  escape_essid((const char*)priv->essid,
							       priv->essid_len),
						  MAC_ARG(priv->bssid));
					break;
			...
			case CMAS_INIT:{
						...
						*resp;
						resp =
						    (struct
						     ieee80211_assoc_response
						     *)&notif->u.raw;
						IWI_DEBUG(							  "association failed (0x%04X): %s\n",
							  ntohs(resp->status),
							  ipw_get_status_code
							  (ntohs
							   (resp->status)));
					...
					IWI_DEBUG(						  "disassociated: '%s' %02x:%02x:%02x:%02x:%02x:%02x \n",
						  escape_essid((const char*)priv->essid,
							       priv->essid_len),
						  MAC_ARG(priv->bssid));
	...
	case HOST_NOTIFICATION_STATUS_AUTHENTICATE:{
			...
			case CMAS_INIT:
				if (priv->status & STATUS_AUTH) {
					IWI_DEBUG(						  "authentication failed (0x%04X): %s\n",
						  ntohs(auth->status),
						  ipw_get_status_code(ntohs
								      (auth->
								       status)));
				}
				IWI_DEBUG(					  "deauthenticated: '%s' %02x:%02x:%02x:%02x:%02x:%02x\n",
					  escape_essid((const char*)priv->essid,
						       priv->essid_len),
					  MAC_ARG(priv->bssid));
	...
	case HOST_NOTIFICATION_STATUS_FRAG_LENGTH:{
			...
			if (notif->size == sizeof(*x))
				IWI_DEBUG("Frag length: %d\n",
					  le16_to_cpu(x->frag_length));
			else
				IWI_DEBUG("Frag length of wrong size %d "
					  "(should be %zd)\n",
					  notif->size, sizeof(*x));
			break;
		}
	...
	case HOST_NOTIFICATION_STATUS_LINK_DETERIORATION:{
			...
			if (notif->size == sizeof(*x)) {
				IWI_DEBUG(					  "link deterioration: type %d, cnt %d\n",
					  x->silence_notification_type,
					  x->silence_count);
				memcpy(&priv->last_link_deterioration, x,
				       sizeof(*x));
			} else {
				IWI_DEBUG("Link Deterioration of wrong size %d "
					  "(should be %zd)\n",
					  notif->size, sizeof(*x));
			}
			break;
	...
	case HOST_NOTIFICATION_NOISE_STATS:{
			IWI_DEBUG("HOST_NOTIFICATION_NOISE_STATS\n");
	...
	default:
		IWI_DEBUG("Unknown notification: "
				   ...
				   struct net_device *dev, int pri)
{
	...
	IWI_DEBUG_FN("dev->xmit(%d bytes)\n", txb->payload_size);
	//spin_lock_irqsave(&priv->lock, flags);

	if (!(priv->status & STATUS_ASSOCIATED)) {
		IWI_DEBUG("Tx attempt while not associated.\n");
...
struct ieee80211_txb * darwin_iwi2200::mbuf2txb(mbuf_t m)
{
	...
	int i, bytes_per_frag, nr_frags, bytes_last_frag, frag_size,
	    rts_required;
	unsigned long flags;
	struct net_device_stats *stats = &priv->ieee->stats;
...
/*	 = {// Ensure zero initialized 
	...
	for ( mn = m; mn; mn = mbuf_next(mn) )
	{
	    m_allsize += mbuf_len(mn); 
	}
	
	IWI_DEBUG("mbuf_len first %d all %d \n"); //, mbuf_len(m), m_allsize);
	
	if (ipw_net_is_queue_full( (struct  net_device *)NULL  ,priority)){
		IWI_DEBUG_FN( " tx queue is full \n");
		//return NETDEV_TX_BUSY;
		return NULL;
	}
	if (unlikely( mbuf_len(m) < SNAP_SIZE + sizeof(u16) ) ) {
		IWI_DEBUG( "%s: skb too small (%d).\n",
		       ieee->dev->name, mbuf_len(m));
		goto success;
	} 
	ether_type = ntohs(((struct ethhdr *)mbuf_datastart(m))->h_proto);
	...
	memcpy(dest, mbuf_datastart(m), ETH_ALEN);
	memcpy(dest,(UInt8*)mbuf_datastart(m) + ETH_ALEN, ETH_ALEN);
	...
	/* fix me: if qos is enable, change follow
	...
	skb_pull(m, sizeof(struct ethhdr));
	bytes = m_allsize - sizeof(struct ethhdr) + SNAP_SIZE + sizeof(u16);
	
	/* Encrypt msdu first on the whole data packet. */
	if ((host_encrypt || host_encrypt_msdu) &&
	    crypt && crypt->ops && crypt->ops->encrypt_msdu) {
		IWI_DEBUG("encrypt is not supported\n");
		goto failed;
#if 0
		int res = 0;
	...
	if (host_encrypt || ieee->host_open_frag) {
		IWI_DEBUG("%s fragmentation is not supported\n",__FUNCTION__);
		goto failed;
#if 0	
	...
	(void*)txb = kmalloc(sizeof(struct ieee80211_txb), NULL);
	if (txb) {
		memset(txb, 0, sizeof(struct ieee80211_txb));
		txb->nr_frags = nr_frags;
		txb->frag_size = frag_size;
		if (txb->nr_frags != 1) {
			IWI_DEBUG("fragmentation is not supported\n");
			goto failed;
		}
		//txb->fragments[0]  = __dev_alloc_skb(txb_size + headroom,gfp_mask);
		//skb_reserve
		txb->fragments[0] = allocatePacket(frag_size + ieee->tx_headroom + 16 );
		//fix me: should check if allocated or not.
		mbuf_adj(txb->fragments[0], 16 + ieee->tx_headroom );
	}
	if (unlikely(!txb)) {
	...
	if (rts_required) {
		IWI_DEBUG("rts is not supported\n");
		goto failed;
#if 0
	...
	for (; i < nr_frags; i++) {
		...
		memcpy((void *)skb_put(skb_frag, bytes), mbuf_datastart(m), bytes);
...
int darwin_iwi2200::ipw_tx_skb(struct ipw_priv *priv, struct ieee80211_txb *txb, int pri)
{
	...
	if (txb->encrypted && !priv->ieee->host_encrypt) {
		...
		case SEC_LEVEL_0:
			break;
		default:
			IWI_ERR("Unknow security level %d\n",
			       priv->ieee->sec.level);
			break;
		}
	} else
	...
	tfd->u.data.num_chunks = cpu_to_le32(min((u8) (NUM_TFD_CHUNKS - 2),
						 txb->nr_frags));
	IWI_DEBUG_FN("%d fragments being sent as %d chunks.\n",
		       txb->nr_frags, le32_to_cpu(tfd->u.data.num_chunks));
	for (i = 0; i < le32_to_cpu(tfd->u.data.num_chunks); i++) {
		IWI_DEBUG_FN("Adding fragment %d of %d (%d bytes).\n",
			       i, le32_to_cpu(tfd->u.data.num_chunks),
			       mbuf_len(txb->fragments[i]) - hdr_len);
		IWI_DEBUG_FN("Dumping TX packet frag %d of %d (%d bytes):\n",
		...
		tfd->u.data.chunk_ptr[i] =
			...
			mbuf_datastart( 
				(mbuf_t)( (UInt8*)txb->fragments[i] + hdr_len ) ))); // add kazu
/*		    cpu_to_le32(
			pci_map_single
				(priv->pci_dev,
				 ...
				 txb->fragments[i]->len - hdr_len,
				 PCI_DMA_TODEVICE)); */
	         // fix me: must get fragment length from IOMbufLittleMemoryCursor::getPhysicalSegmentsWithCoalesce
		tfd->u.data.chunk_len[i] =
		     cpu_to_le16(mbuf_len(txb->fragments[i]) - hdr_len);
	...
	if (i != txb->nr_frags) {
		...
		IWI_DEBUG_FN( "Trying to reallocate for %d bytes\n",
		       remaining_bytes);
	...
	return NETDEV_TX_OK;
      ...
      drop:
	IWI_DEBUG_FN("Silently dropping Tx packet.\n");
	// ieee80211_txb_free(txb);  must research
	return NETDEV_TX_OK;
}
...
UInt32 darwin_iwi2200::outputPacket(mbuf_t m, void * param) {
	...
	if(txb){
		IWI_DEBUG_FN("mbuf2txb is successed\n");
		if(ipw_net_hard_start_xmit(txb,priv->net_dev,1) == NETDEV_TX_OK)
			IWI_DEBUG_FN("tx packet is added to queue\n");
		else
			IWI_DEBUG_FN("tx packet is not added to queue\n");
		//kfree(txb);
