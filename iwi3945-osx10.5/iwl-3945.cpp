				...
				struct iwl_rx_mem_buffer *rxb)
{
	...
	struct ieee80211_rx_status stats;// = {
		stats.mactime = le32_to_cpu(rx_end->beacon_timestamp);
		stats.freq = ieee80211chan2mhz(le16_to_cpu(rx_hdr->channel));
		stats.channel = le16_to_cpu(rx_hdr->channel);
		stats.phymode = (rx_hdr->phy_flags & RX_RES_PHY_FLAGS_BAND_24_MSK) ?
	...
	//			 le32_to_cpu(bd->pa[i].len), PCI_DMA_TODEVICE);
		...
		if (txq->txb[txq->q.last_used].skb[0]) {
			...
			if (txq->txb[txq->q.last_used].skb[0]) {
				...
				if (txq->txb[txq->q.last_used].skb[0])
				if (!(mbuf_type(txq->txb[txq->q.last_used].skb[0]) == MBUF_TYPE_FREE) ) mbuf_freem(txq->txb[txq->q.last_used].skb[0]);
...
int iwl_hw_nic_init(struct iwl_priv *priv)
{
	...
	if (!rxq->bd) {
		...
		if (rc) {
			...
			IWL_DEBUG_INFO("Rx queue [ALOCATED]\n");
		}
	} else{
		IWL_DEBUG_INFO("Rx queue [RESETED]\n");
...
void iwl3945_reg_txpower_periodic(struct iwl_priv *priv)
{
 ...
 reschedule:
	...
	clone->queue_te(11,OSMemberFunctionCast(thread_call_func_t,clone,&darwin_iwi3945::iwl3945_bg_reg_txpower_periodic),priv,REG_RECALIB_PERIOD * HZ,true);
}

void darwin_iwi3945::iwl3945_bg_reg_txpower_periodic(struct iwl_priv *priv)
{
...
//	};
	...
	rate_cmd.table_id = 1;
	return iwl_send_cmd_pdu(priv, REPLY_RATE_SCALE, sizeof(rate_cmd), &rate_cmd);
}
...
{
	INIT_DELAYED_WORK(&priv->thermal_periodic, iwl3945_bg_reg_txpower_periodic);
}
