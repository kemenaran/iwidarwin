...
#include "firmware/iwi_ucode.fw.h"
#include "firmware/iwi_firm.fw.h"
#include "defines.h"
...
int darwin_iwi3945::ipw_load_ucode(struct ipw_priv *priv)
{
	...
	rc = ipw_download_ucode_base(priv, (u8*)iwi_boot, sizeof(iwi_boot));
	//			     &ucode->data[ucode->inst_size +
	//					  ucode->data_size],
	//			     ucode->boot_size);
	if (rc)
	...
	rc = ipw_download_ucode(priv, (struct fw_image_desc*)iwi_ucode,
	//&(priv->ucode_code),
				ALM_RTC_INST_SIZE, RTC_INST_LOWER_BOUND);
	if (rc)
		goto error;

	rc = ipw_download_ucode(priv, (struct fw_image_desc*)iwi_firm,
	// &(priv->ucode_data),
