			  ...
			  struct iwl_link_quality_cmd *lq, u8 flags)
{
	...
	struct iwl_host_cmd cmd;// = {
		cmd.id = REPLY_TX_LINK_QUALITY_CMD;
		cmd.len = sizeof(struct iwl_link_quality_cmd);
			 ...
			 struct ieee80211_tx_status *tx_resp)
{
	...
	struct ieee80211_local *local = (struct ieee80211_local*)(dev->ieee80211_ptr);
	struct iwl_rate_scale_data *window = NULL;
...
static void *rs_alloc_sta(void *priv, int gfp)
{
	struct iwl_rate_scale_priv *crl;
	int i, j;

	IWL_DEBUG_RATE("create station rate scale window\n");
	(void*)crl = kmalloc(sizeof(struct iwl_rate_scale_priv),0);
			 ...
			 struct sta_info *sta)
{
	...
	struct ieee80211_hw_mode *mode = local->oper_hw_mode;
	struct iwl_priv *priv = (struct iwl_priv *)priv_rate;
	struct iwl_rate_scale_priv *crl = (struct iwl_rate_scale_priv*)priv_sta;
...
{
	struct iwl_rate_scale_priv *rs_priv = (struct iwl_rate_scale_priv*)priv_sta;
