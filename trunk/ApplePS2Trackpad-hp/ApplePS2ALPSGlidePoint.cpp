...
// IOLog(" %s() " fmt, __FUNCTION__, ##__VA_ARGS__)
...
bool ApplePS2ALPSGlidePoint::start( IOService * provider )
{
  ...
  //  setTapEnable( true );
    ...
    _device->installInterruptAction(this, OSMemberFunctionCast(
            PS2InterruptAction, this, 
            &ApplePS2ALPSGlidePoint::interruptOccurred) );
	...
	_device->installPowerControlAction( this, OSMemberFunctionCast(
            PS2PowerControlAction, this, 
            &ApplePS2ALPSGlidePoint::setDevicePowerState) );
	_powerControlHandlerInstalled = true;

	_device->registerService();
    return true;
}
...
void ApplePS2ALPSGlidePoint::dispatchRelativePointerEventWithPacket( 
        UInt8 * packet,
        UInt32  packetSize
    )
{
    ...
    //DEBUG_LOG("Relative packet: dx:%d, dy:%d, buttons:%d\n",
    //    (int)dx, (int)dy, (int)buttons);	
    dx = -1;
    dy =  1;
    dispatchRelativePointerEvent(dx, -dy, buttons, t);
}
...
IOReturn ApplePS2ALPSGlidePoint::setParamProperties( OSDictionary * dict )
{
	...
	OSNumber * speed  = OSDynamicCast( OSNumber, dict->getObject("HIDTrackpadAcceleration") );
	...
	if (speed)
    {
       setProperty("HIDTrackpadAcceleration", speed);
    }
