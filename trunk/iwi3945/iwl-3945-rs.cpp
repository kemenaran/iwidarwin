...
{
	struct iwl_rate_scale_priv *rs_priv = (struct iwl_rate_scale_priv*)data;
	int unflushed = 0;
	unsigned long flags;
...
/*              duration = jiffies_to_msecs(rs_priv->flush_time); */
		...
		//mod_timer(&rs_priv->rate_scale_flush, jiffies + rs_priv->flush_time);
...
static void rs_free_sta(void *priv, void *priv_sta)
{
	...
	IOFree(rs_priv, sizeof(struct iwl_rate_scale_priv));
	IWL_DEBUG_RATE("leave\n");
}
