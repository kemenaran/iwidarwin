 ...
 * adjusting ...
 */
int no_sleep_autoadjust = true;
//module_param(no_sleep_autoadjust, int, S_IRUGO);
				     ...
				     int dynps_ms, int wakeup_period)
{
	...
	for (i = 0; i < IWL_POWER_VEC_SIZE; i++)
		cmd->sleep_interval[i] =wakeup_period;
...
int iwl_power_update_mode(struct iwl_priv *priv, int force)
{
	int ret = 0;
	struct iwl_tt_mgmt *tt = &priv->thermal_throttle;
	int enabled = (priv->iw_mode == NL80211_IFTYPE_STATION) &&
...
static void iwl_tt_check_exit_ct_kill(unsigned long data)
{
	...
	if (tt->state == IWL_TI_CT_KILL) {
		...
		mod_timer(&priv->thermal_throttle.ct_kill_exit_tm,  CT_KILL_EXIT_DURATION * HZ);
	}
}
 ...
 */
static void iwl_legacy_tt_handler(struct iwl_priv *priv, s32 temp, int force)
{
	struct iwl_tt_mgmt *tt = &priv->thermal_throttle;
 ...
 *=============================================================================
 */
static void iwl_advance_tt_handler(struct iwl_priv *priv, s32 temp, int force)
{
	struct iwl_tt_mgmt *tt = &priv->thermal_throttle;
	int i;
	int changed = false;
