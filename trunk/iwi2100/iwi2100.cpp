...
int darwin_2100::ipw2100_download_firmware(struct ipw2100_priv *priv)
{
	...
	case IW_MODE_ADHOC:
		ipw2100_firmware=(struct ipw2100_fw*)iwi_ibss;
		break;
	case IW_MODE_MONITOR:
		ipw2100_firmware=(struct ipw2100_fw*)iwi_mon;
		break;
	case IW_MODE_INFRA:
	default:
		ipw2100_firmware=(struct ipw2100_fw*)iwi_bss;
		break;
	}
...
{
	IOLog("trying to down the interface\n");
	if (pl==0)
	{
	super::disable(netif);
	//stop(NULL);
							 ...
							 struct apple80211_status_dev_data *dd)
{
	//char i[4];
	//sprintf(i,"en1");
	//memcpy(dd->dev_name,i,sizeof(i));
	IOLog("getSTATUS_DEV %s\n",dd->dev_name);
	ifnet_find_by_name("en1",&fifnet);
	IOLog("ifnet_t %s = %x\n",ifnet_name(fifnet),fifnet);
	ipw2100_initialize_ordinals(priv);
	ipw2100_sw_reset(1);

	ipw2100_load(priv);
	//ipw2100_init_ordinals(priv);
	//configu(priv);
	super::enable(fNetif);
