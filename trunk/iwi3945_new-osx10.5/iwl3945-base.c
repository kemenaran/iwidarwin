...
static int iwl3945_send_rxon_assoc(struct iwl3945_priv *priv)
{
	...
	res = (struct iwl3945_rx_packet *)skb_data(cmd.meta.u.skb);
				     ...
				     struct iwl3945_cmd *cmd, struct sk_buff *skb)
{
	...
	res = (struct iwl3945_rx_packet *)skb_data(skb);
	if (res->hdr.flags & IWL_CMD_FAILED_MSK) {
				    ...
				    u16 phy_flags)
{
	...
	iwl3945_rt = (void *)skb_data(rxb->skb);
...
static void iwl3945_tx_cmd_complete(struct iwl3945_priv *priv,
				struct iwl3945_rx_mem_buffer *rxb)
{
	struct iwl3945_rx_packet *pkt = (struct iwl3945_rx_packet *)skb_data(rxb->skb);
	u16 sequence = le16_to_cpu(pkt->hdr.sequence);
	int txq_id = SEQ_TO_QUEUE(sequence);
	int index = SEQ_TO_INDEX(sequence);
...
static void iwl3945_rx_handle(struct iwl3945_priv *priv)
{
	...
	while (i != r) {
		...
		pci_dma_sync_single_for_cpu(priv->pci_dev, rxb->dma_addr,
					    IWL_RX_BUF_SIZE,
					    PCI_DMA_FROMDEVICE);
		pkt = (struct iwl3945_rx_packet *)skb_data(rxb->skb);
...
static void iwl3945_bg_alive_start(struct iwl3945_priv *priv)
{
	...
	mutex_lock(&priv->mutex);
	IOLog("__ IN iwl3945_alive_start __\n");
	iwl3945_alive_start(priv);
	IOLog("__ OUT iwl3945_alive_start __\n");
	mutex_unlock(&priv->mutex);
}
				 ...
				 struct ieee80211_if_init_conf *conf)
{
	...
	else IWL_DEBUG_MAC80211("failed to set mode\n");
...
static void iwl3945_setup_deferred_work(struct iwl3945_priv *priv)
{
	...
	INIT_WORK(&priv->scan_completed, iwl3945_bg_scan_completed,3);
	INIT_WORK(&priv->request_scan, iwl3945_bg_request_scan,4);
	INIT_WORK(&priv->abort_scan, iwl3945_bg_abort_scan,5);
	INIT_WORK(&priv->rf_kill, iwl3945_bg_rf_kill,6);
	INIT_WORK(&priv->beacon_update, iwl3945_bg_beacon_update,21);
	INIT_DELAYED_WORK(&priv->post_associate, iwl3945_bg_post_associate,7);
	INIT_DELAYED_WORK(&priv->init_alive_start, iwl3945_bg_init_alive_start,8);
	INIT_DELAYED_WORK(&priv->alive_start, iwl3945_bg_alive_start,9);
...
module_scan(iwl3945_scan_initiate);
