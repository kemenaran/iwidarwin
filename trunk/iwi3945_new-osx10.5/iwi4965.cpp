...
OSDefineMetaClassAndStructors(darwin_iwi4965, IO80211Controller);
#else
#define super IOEthernetController
OSDefineMetaClassAndStructors(darwin_iwi4965, IOEthernetController);
#endif
...
extern "C" {
    ...
    extern int (*iwlready)(struct iwl4965_priv *);
	...
	extern void (*iwl_down)(struct iwl4965_priv *);
	extern void (*iwl_up)(struct iwl4965_priv *);
...
IOService *darwin_iwi4965::getProvider() {
    return my_provider;
}
...
IOOutputQueue * darwin_iwi4965::createOutputQueue( void )
{
    ...
    return IOBasicOutputQueue::withTarget(this,(IOOutputAction)&darwin_iwi4965::outputPacket2,0);
}

int darwin_iwi4965::outputRaw80211Packet( IO80211Interface * interface, mbuf_t m )
{
	return -1;
...
UInt32 darwin_iwi4965::getFeatures() const {
    return kIONetworkFeatureSoftwareVlan;
}


#ifdef IO80211_VERSION
void darwin_iwi4965::postMessage(UInt32 message) {
...
int configureConnection(kern_ctl_ref ctlref, u_int unit, void *userdata, int opt, void *data, size_t len)
{
	if (opt==1)
	{
		if (test_bit(3, &clone->priv->status)) // off -> on 3=STATUS_RF_KILL_SW
		{
			...
			if(get_my_priv())
				iwl_up((struct iwl4965_priv*)get_my_priv());
			else
				IOLog("No Priv\n");
		...
		else
		{
			...
			if(get_my_priv())
				iwl_down((struct iwl4965_priv*)get_my_priv());
			else
				IOLog("No Priv\n");
		}	
	}
	if(opt == 2){
		IOLog("Scan requested\n");
...
bool darwin_iwi4965::start(IOService *provider)
{
	...
	do {
		...
		queue_te2(0,OSMemberFunctionCast(thread_call_func_t,this,&darwin_iwi4965::check_firstup),NULL,2000,true);
...
void darwin_iwi4965::queue_td2(int num , thread_call_func_t func)
{
	if (tlink2[num])
	{
...
void darwin_iwi4965::queue_te2(int num, thread_call_func_t func, thread_call_param_t par, UInt32 timei, bool start)
{
		//par=my_hw->priv;
...
void darwin_iwi4965::check_firstup(void)
{
	IOLog("check_firstup\n");
	if (first_up==0) 
	{
...
SInt32 
darwin_iwi4965::getMCS_INDEX_SET(IO80211Interface *interface,
...
SInt32
darwin_iwi4965::getHARDWARE_VERSION(IO80211Interface *interface,
...
SInt32
darwin_iwi4965::getDRIVER_VERSION(IO80211Interface *interface,
                                    struct apple80211_version_data *hv)
{
    hv->version = APPLE80211_VERSION;
...
SInt32
darwin_iwi4965::setCHANNEL(IO80211Interface *interface,
...
SInt32
darwin_iwi4965::getPOWERSAVE(IO80211Interface *interface,
...
SInt32
darwin_iwi4965::setPOWERSAVE(IO80211Interface *interface,
                             apple80211_powersave_data *psd)
{
...
SInt32 darwin_iwi4965::getASSOCIATE_RESULT( IO80211Interface * interface, 
							 ...
							 struct apple80211_status_dev_data *dd)
{
    ...
    strncpy((char*)dd->dev_name, "iwi4965", sizeof(dd->dev_name));
...
SInt32 
darwin_iwi4965::getLOCALE(IO80211Interface *interface, apple80211_locale_data *ld)
{
...
SInt32 
darwin_iwi4965::getCOUNTRY_CODE(IO80211Interface *interface, apple80211_country_code_data *cd) {
    cd->version = APPLE80211_VERSION;
...
SInt32
darwin_iwi4965::getPHY_MODE(IO80211Interface *interface, apple80211_phymode_data *pd)
{
    pd->version = APPLE80211_VERSION;
...
SInt32 
darwin_iwi4965::getINT_MIT(IO80211Interface *interface, apple80211_intmit_data *mitd)
{
...
SInt32 
darwin_iwi4965::getTXPOWER(IO80211Interface *interface, apple80211_txpower_data *tx)
{
...
SInt32 
darwin_iwi4965::getOP_MODE(IO80211Interface *interface, apple80211_opmode_data *od)
{
...
SInt32 
darwin_iwi4965::getNOISE(IO80211Interface *interface, apple80211_noise_data *nd)
{
...
SInt32
darwin_iwi4965::getSCAN_RESULT(IO80211Interface *interface, apple80211_scan_result **sr)
{
...
SInt32 
darwin_iwi4965::setRATE(IO80211Interface *interface, apple80211_rate_data *rd)
{
...
SInt32 
darwin_iwi4965::getSUPPORTED_CHANNELS(IO80211Interface *interface, apple80211_sup_channel_data *ad) {
...
SInt32 
darwin_iwi4965::getTX_ANTENNA(IO80211Interface *interface, apple80211_antenna_data *ad)
{
    ad->version = APPLE80211_VERSION;
    ad->num_radios = 3;
...
SInt32 
darwin_iwi4965::getANTENNA_DIVERSITY(IO80211Interface *interface, apple80211_antenna_data *ad)
{
    ad->version = APPLE80211_VERSION;
    ad->num_radios = 3;
    ad->antenna_index[0] = 1;
...
SInt32
darwin_iwi4965::getSTATION_LIST(IO80211Interface *interface, apple80211_sta_data *sd)
{
    int i;
...
SInt32 
darwin_iwi4965::setANTENNA_DIVERSITY(IO80211Interface *interface, apple80211_antenna_data *ad)
{
...
SInt32 
darwin_iwi4965::setTX_ANTENNA(IO80211Interface *interface, apple80211_antenna_data *ad)
{
...
SInt32 
darwin_iwi4965::setTXPOWER(IO80211Interface *interface, apple80211_txpower_data *td)
{
...
SInt32 
darwin_iwi4965::setINT_MIT(IO80211Interface *interface, apple80211_intmit_data *md)
{
...
SInt32 
darwin_iwi4965::getPROTMODE(IO80211Interface *interface, apple80211_protmode_data *pd)
{
	pd->version = APPLE80211_VERSION;
...
SInt32 
darwin_iwi4965::setPROTMODE(IO80211Interface *interface, apple80211_protmode_data *pd)
{
...
SInt32
darwin_iwi4965::setPHY_MODE(IO80211Interface *interface, apple80211_phymode_data *pd)
{
...
SInt32
darwin_iwi4965::setLOCALE(IO80211Interface *interface, apple80211_locale_data *ld) {
...
SInt32
darwin_iwi4965::setASSOCIATE(IO80211Interface *interface,struct apple80211_assoc_data *ad)
{
	IOLog("setASSOCIATE \n");
...
SInt32
darwin_iwi4965::getAUTH_TYPE(IO80211Interface *interface,
...
SInt32 darwin_iwi4965::apple80211Request( UInt32 req, int type, IO80211Interface * intf, void * data ) {
    SInt32 ret = 0;
 ...
 * thread.
 *-------------------------------------------------------------------------*/
 UInt32 darwin_iwi4965::outputPacket2(mbuf_t m, void * param)
{
	IOLog("outputPacket2 called\n");
...
/*SInt32 darwin_iwi4965::apple80211_ioctl(
                                        IO80211Interface *interface, 
                                        ifnet_t ifn, 
                                        u_int32_t cmd, 
                                        void *data)
{
    IOLog("darwin_iwi4965::apple80211_ioctl(%d, %d, %p)\n", ifn, cmd, data);
    return super::apple80211_ioctl(interface, ifn, cmd, data);
}*/

IOReturn darwin_iwi4965::setMulticastMode(bool active) {

	return kIOReturnSuccess;
}

IOReturn darwin_iwi4965::setMulticastList(IOEthernetAddress * addrs, UInt32 count) {
	 return kIOReturnSuccess;
}

IOReturn darwin_iwi4965::getPacketFilters(const OSSymbol * group, UInt32 *         filters) const
{
...
IOReturn darwin_iwi4965::enablePacketFilter(const OSSymbol * group,
                                        UInt32           aFilter,
...
IOReturn darwin_iwi4965::getMaxPacketSize(UInt32 * maxSize) const
{
    *maxSize = 1600;//kIOEthernetMaxPacketSize;//;//IPW_RX_BUF_SIZE;
    return kIOReturnSuccess;
}

IOReturn darwin_iwi4965::getMinPacketSize(UInt32 * minSize) const
{
...
{
    IOLog("darwin_iwi4965::configureInterface()\n");
    IONetworkData * data;
...
#ifdef IO80211_VERSION
IO80211Interface *darwin_iwi4965::getNetworkInterface()
{
...
/*static IOReturn darwin_iwi4965::powerChangeHandler(void *target, void *refCon, UInt32
...
static IOReturn darwin_iwi4965::powerDownHandler(void *target, void *refCon, UInt32
            messageType, IOService *service, void *messageArgument,
            vm_size_t argSize ) {
